...
type CommonMsg struct {
	...
	Utimes       int     `bson:"times,omitempty" json:"times,omitempty"` // 次数
	Upage        string  `bson:"page" json:"page"`                       // 页面路径
	Uhash        string  `bson:"hash" json:"hash"`                       // 页面hash
	Version      string  `bson:"version" json:"v"`                       // 版本
	Uenv         string  `bson:"env" json:"e"`                           // 开发生产环境
	AppKey       string  `bson:"appkey,omitempty" json:"token"`          // 项目id
	Udevice      string  `bson:"device" json:"device"`                   // 运行环境平台的信息
	Begin        int     `bson:"begin" json:"begin"`                     // 开始时间戳
	...
	Uuser        string  `bson:"userinfo" json:"user"`                   // 自定义 的user info
	// 内部信息转换用的
